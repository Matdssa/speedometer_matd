	...
	while true do
		...
		if(IsPedInAnyVehicle(Ped)) then
			...
			if PedCar and GetPedInVehicleSeat(PedCar, -1) == Ped then
				...
				-- Clignotant	(Touche Modifiable)
				-- SetVehicleIndicatorLights (1 gauche -- 0 droite)
				local VehIndicatorLight = GetVehicleIndicatorLights(GetVehiclePedIsUsing(PlayerPedId()))
				if IsControlJustPressed(1, 57) then -- Touche de base (F9) 
					ind.l = not ind.l
					SetVehicleIndicatorLights(GetVehiclePedIsUsing(GetPlayerPed(-1)), 0, ind.l)
				end
				if IsControlJustPressed(1, 56) then -- Touche de base (F10)
					ind.r = not ind.r
...
-- Carburant + taux de consommation
Citizen.CreateThread(function()
	while true do
		local Ped = GetPlayerPed(-1)
		if(IsPedInAnyVehicle(Ped)) then
